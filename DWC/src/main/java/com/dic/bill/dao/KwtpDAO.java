...
public interface KwtpDAO extends JpaRepository<Kwtp, Integer> {
    ...
    @Query(value = "select distinct t from Kwtp t join t.kart k join k.kartExt e join k.nabor n " +
            " where n.org.id=:orgId and t.dtInk between :genDt1 and :genDt2 order by t.id")
    List<Kwtp> getKwtpKartExtByReuWithLsk(@Param("orgId") Integer orgId,
     ...
     * @param genDt2 - дата окончания
     */
    @Query(value = "select distinct t from Kwtp t join t.kart k join k.koKw.kartExtByKoKw e join k.nabor n " +
            " where n.org.id=:orgId and t.dtInk between :genDt1 and :genDt2 order by t.id")
    List<Kwtp> getKwtpKartExtByReuWithKoKw(@Param("orgId") Integer orgId,
                                         @Param("genDt1") Date genDt1, @Param("genDt2") Date genDt2);
    ...
    @Query(value = "select distinct t from Kwtp t join t.kart k join k.koPremise.kartExtByPremise e join k.nabor n " +
            " where n.org.id=:orgId and t.dtInk between :genDt1 and :genDt2 order by t.id")
    List<Kwtp> getKwtpKartExtByReuWithPremise(@Param("orgId") Integer orgId,
                                              @Param("genDt1") Date genDt1, @Param("genDt2") Date genDt2);
