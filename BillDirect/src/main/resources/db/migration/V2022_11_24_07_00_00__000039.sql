create or replace package body scripts is
...
l_dt:=to_date('20221101','YYYYMMDD');
--комментарий
comment_:='Снятие сальдо и пени по ЧГК';
--Уникальный id переброски
cd_:='swp_sal_nothing_202211_1';
...
from a_penya s, kart k where
        s.lsk=k.lsk
                         and s.mg=l_mg_back
                         and k.reu='018'; -- снять пеню по чгк, где она на отдельных л.с.

insert into c_change (lsk, usl, org, summa, mgchange, type, dtek, ts,
                      user_id, doc_id)
select s.lsk, s.usl, s.org, -1*s.summa as summa,
       '201907', 1, l_dt, sysdate, user_id_, l_id
from SALDO_USL_SCRIPT s, kart k where
        s.lsk=k.lsk and s.mg=l_mg_sal
                                  and s.usl='107'; -- снять сальдо по чгк, по услуге
...
-- ред.25.12.18
procedure dist_saldo_polis(p_dt in date) is
    l_mg           params.period%type;
l_mg3          params.period%type;
l_user         number;
l_id           number;
...
l_last_deb_max      number;
  begin
select p.period into l_mg from params p; --тек.период
--l_mg       := '202012'; --тек.период
l_cd       := 'dist_saldo_polis_'||l_mg;
l_mgchange := l_mg;
l_dt       := to_date(l_mg||to_char(p_dt,'DD'), 'YYYYMMDD'); --число проводки
--l_mg3      := utils.add_months_pr(l_mg, 1); --месяц вперед, по исх сальдо -- до 24.11.22, потом переписал на xitog3_lsk
l_mg3      := l_mg; --текущий месяц, по исх сальдо
                ...
                join usl u2
                 ...
                 and exists (select t.*
                       ...
                       where t.mg = s.mg
                         and t.lsk = s.lsk
                         and t.summa > 0)
              ) loop*/ -- до 24.11.22, потом переписал на xitog3_lsk
...
for c in (select distinct s.lsk, s.mg
                ...
                join usl u2
                  on s.mg = l_mg3
                 and s.usl = u2.usl
                 and nvl(s.outdebet,0)+nvl(s.outkredit,0) < 0 --есть кредит
                    --and s.lsk='06005556'
                 and exists (select t.*
                        from xitog3_lsk t -- где есть дебет.сальдо по другим услугам
                       where t.mg = s.mg
                         and t.lsk = s.lsk
                         and nvl(s.outdebet,0)+nvl(s.outkredit,0) > 0)
      ...
      --найти абс кред и деб сальдо
select abs(nvl(sum(t.outkredit),
               0)),nvl(sum(t.outdebet),
                       0)
into l_kr, l_deb
from xitog3_lsk t
where t.mg = c.mg
...
l_last_deb_max := 0;
      --снять с кредита
l_corr_kr := 0;
for c2 in (select t.lsk, t.usl, t.org, abs((nvl(t.outdebet,0)+nvl(t.outkredit,0))) as sal, round(abs((nvl(t.outdebet,0)+nvl(t.outkredit,0))) *
                               l_coeff2,
                               2) as summa
                   from xitog3_lsk t
                  where t.mg = c.mg
                    and (nvl(t.outdebet,0)+nvl(t.outkredit,0)) < 0
                    and t.lsk = c.lsk
                    and (nvl(t.outdebet,0)+nvl(t.outkredit,0)) * l_coeff2 <> 0) loop
...
--снять с дебета
l_corr_deb := 0;
for c2 in (select t.lsk, t.usl, t.org, (nvl(t.outdebet,0)+nvl(t.outkredit,0)) as sal, round((nvl(t.outdebet,0)+nvl(t.outkredit,0)) *
                               l_coeff,
                               2) as summa
                   from xitog3_lsk t
                  where t.mg = c.mg
                    and (nvl(t.outdebet,0)+nvl(t.outkredit,0)) > 0
                    and t.lsk = c.lsk
                    and (nvl(t.outdebet,0)+nvl(t.outkredit,0)) * l_coeff <> 0) loop
insert into t_corrects_payments
(lsk, usl, org, summa, user_id, dat, mg, dopl, fk_doc, var)
...
-- используя коэффициент и корректировку ошибки
-- ред.21.11.2019
procedure dist_saldo_PEN_polis(p_dt in date) is
    l_mg           params.period%type;
l_mg3          params.period%type;
l_user         number;
l_id           number;
...
l_mgchange := l_mg;
l_dt       := to_date(l_mg||to_char(p_dt,'DD'), 'YYYYMMDD');
l_mg3      := l_mg;
...
-- снять кредитовое сальдо по пене, перенести на оплату (какая то хуета!)
procedure swap_pen_to_pay is
  l_doc number;
l_user_id number;
l_cd varchar2(250);
l_dopl varchar2(6);
l_dat date;
begin
select t.id into l_user_id from t_user t where t.cd='SCOTT';
l_dopl := '202204';
l_dat:=gdt(1,0,0);

delete from t_corrects_payments t where t.mg=l_dopl
                                    and exists (select * from c_change_docs d where d.id=t.fk_doc and
            d.cd_tp=l_cd);
...
(mgchange, dtek, ts, user_id, cd_tp)
values
    (l_dopl, l_dat, sysdate, l_user_id, l_cd)
returning id into l_doc;

for c in (select t.lsk, t.summa, t.mg1 as dopl from c_penya t where t.lsk='06000135' and t.penya<0) loop

for c2 in (select t.usl, t.org  from xitog3_lsk t where t.lsk=c.lsk and nvl(t.poutsal,0)>0 order by t.poutsal desc) loop
   -- снять с первой орг из исх сальдо и поставить на оплату основного долга
insert into t_corrects_payments
(lsk, usl, org, penya, user_id, dat, mg, dopl, fk_doc)
values
    (c.lsk, c2.usl, c2.org, c.summa, l_user_id, l_dat, l_dopl, c.dopl, l_doc);
insert into t_corrects_payments
(lsk, usl, org, summa, user_id, dat, mg, dopl, fk_doc)
values
    (c.lsk, c2.usl, c2.org, -1*c.summa, l_user_id, l_dat, l_dopl, c.dopl, l_doc);
exit;
end loop;
