-- исправил определение даты запрета на получение показаний
...
for c2 in (select 1 as exs, t.id as fk_tp from u_list t where t.cd = p_tp)
                    loop
insert into meter (npp, k_lsk_id, fk_usl, dt1, dt2, fk_klsk_obj)
values (p_npp, c.klsk, p_usl, p_dt1, p_dt2, p_fk_klsk_obj);

select max(decode(u.cd, 'ins_sch', u.id, 0)),
       max(decode(u.cd, 'ins_vol_sch', u.id, 0)),
       max(s.id),
       trim(max(u2.counter)),
       trim(max(k.lsk)),
       max(p.period)
...
if p_met_klsk is not null then
            ...
            for c in (select m.k_lsk_id
                      ...
                      from kart k
                               ...
                               join meter m
                                    on k.k_lsk_id = m.fk_klsk_obj and m.fk_usl = p_usl and k.lsk = p_lsk and case
                                                                                                                 when m.dt1 <=
...
select max(decode(u.cd, 'ins_sch', u.id, 0)),
       ...
       trim(max(k.lsk)),
       max(m.dt2),
       max(to_date(p.period || '01', 'YYYYMMDD')),
       max(p.period)
...
l_flag := 0;
if l_dt2 > l_dt then
            if (nvl(p_vol, 0)) <> 0 then -- всегда брать только расход
            --отразить объем
insert into t_objxpar (fk_k_lsk, fk_list, n1, fk_user, mg, tp)
values (l_met_klsk, l_vol_id, p_vol, l_user, l_period, p_tp);

select nvl(m.n1, 0) + nvl(p_vol, 0) into l_cnt from meter m where m.k_lsk_id = l_met_klsk;
...
execute immediate 'update kart k set k.' || l_counter || '=nvl(k.' || l_counter || ',0)+nvl(' ||
                                  p_vol || ',0) where k.lsk=:lsk' using l_lsk;
--меняем первую букву на "m" - получаем поле для расхода (изврат)
execute immediate 'update kart k set k.' || 'm' || substr(l_counter, 2, 3) || '=nvl(k.' || 'm' ||
...
for c in (select min(x.id) as min_id, x.fk_k_lsk
                  from t_doc d
                           join t_objxpar x on d.id = x.fk_doc and d.id = p_doc_par_id
                  where d.mg = l_period
                  group by x.fk_k_lsk)
            loop
...
if sql%rowcount > 0 then l_cnt := l_cnt + 1; end if;
end loop;
...
--проверить показание по счетчику
procedure check_data_meter(p_met_klsk in number default null, -- klsk счетчика
...
if p_met_klsk is null and p_meter_id is null then
            raise_application_error(-20000,
                                    'Некорректное использование функции p_meter.check_data_meter, p_met_klsk или p_meter_id - пустое!');
end if;
l_day_restrict_meter_vol := utils.get_int_param('DAY_RESTRICT_METER_VOL');
        -- если l_day_restrict_meter_vol=0, то без ограничений
if l_day_restrict_meter_vol != 0 and trunc(sysdate) >= to_date(l_period || case
                                                                                       when l_day_restrict_meter_vol >= 10
                                                                                           then to_char(l_day_restrict_meter_vol)
                                                                                       else '0' || to_char(l_day_restrict_meter_vol) end,
                                                                       'YYYYMMDD') then
...
for c in (select k.lsk, m.id, m.fk_usl, trim(u3.counter) as counter, u.id as vol_id, u2.id as num_id
                  from meter m
                           join kart k on m.fk_klsk_obj = k.k_lsk_id
                           join v_lsk_tp tp on k.fk_tp = tp.id and coalesce(p_lsk, k.lsk) = k.lsk and
                                               case when p_lsk is null then 'LSK_TP_MAIN' else tp.cd end =
                                               tp.cd -- или указан лиц.сч или основной лиц.сч. к которому привязан счетчик
                      and k.psch not in (8, 9)
            ...
            loop
                for c2 in (select t.n1
                           from t_objxpar t
                                    join u_list u on t.fk_list = u.id and u.cd = 'ins_sch'
                                    --join params p on t.mg < p.period
                           where t.fk_k_lsk = l_met_klsk -- последние показания счетчика
                           order by t.id desc)
                    loop
                        l_last_n1 := c2.n1;
l_n1 := l_last_n1;
...
-- получить объем
/*
отключил, так как в check_data_meter эта проверка не нужна (еще не пришло начальное показание)
l_vol := nvl(p_n1, 0) - nvl(l_n1, 0);
if (c.fk_usl in ('011', '015') and abs(l_vol) > 50 or c.fk_usl in ('038') and abs(l_vol) >
       1500) then
...
for c in (select m.k_lsk_id, nvl(m.n1, 0) as prev_val
                  from meter m
                           join params p on 1 = 1
                           join kart k on k.lsk = p_lsk and m.fk_klsk_obj = k.k_lsk_id and m.fk_usl = p_usl and
                                          last_day(to_date(trim(l_period) || '01', 'YYYYMMDD')) between m.dt1 and m.dt2 -- активный счетчик на конец месяца
            )
            loop
                check_data_meter(p_met_klsk => c.k_lsk_id, p_n1 => p_cur_val, p_ret => l_ret);
if l_ret!=0 then
                      p_ret := l_ret;
exit;
end if;
...
if p_is_set_prev = 1 then
                    -- установить предыдущие показания (отматать назад или вперед текущие), без контроля значений, шаг #1
                    ins_data_meter(p_met_klsk => c.k_lsk_id, p_n1 => p_prev_val, p_ts => p_ts, p_period => l_period,
                                   p_status => p_status, p_user => p_user, p_control => 0, p_is_set_prev =>1,
                                   p_doc_par_id => p_doc_par_id, p_ret => l_ret);
                    -- установить текущие показания, с контролем значений, но не знака (+-) (ред. 27.02.20 Кис.просят чтобы можно было отматывать счетчик назад), шаг #2
ins_data_meter(p_met_klsk => c.k_lsk_id, p_n1 => p_cur_val, p_ts => p_ts, p_period => l_period,
                  p_status => p_status, p_user => p_user, p_control => 2, p_doc_par_id => p_doc_par_id,
                  p_ret => l_ret);
else
                    ...
                    ins_data_meter(p_met_klsk => c.k_lsk_id, p_n1 => p_cur_val, p_ts => p_ts, p_period => l_period,
                                   p_status => p_status, p_user => p_user,
...
l_day_restrict_meter_vol := utils.get_int_param('DAY_RESTRICT_METER_VOL');
        -- если l_day_restrict_meter_vol=0, то без ограничений
if l_day_restrict_meter_vol != 0 and trunc(sysdate) >= to_date(to_char(sysdate,'YYYYMM') || case
                                                                                       when l_day_restrict_meter_vol >= 10
                                                                                           then to_char(l_day_restrict_meter_vol)
                                                                                       else '0' || to_char(l_day_restrict_meter_vol) end,
                                                                       'YYYYMMDD') then
...
if p_is_set_prev = 1 or p_n1 >= l_last_n1 then -- обработка новых показаний (предыдущих показаний, если p_is_set_prev = 1)
-- флаг, что не надо добавлять информацию в триггере
                    g_flag := 1;
...
if l_vol <> 0 then
insert into t_objxpar (fk_k_lsk, fk_list, n1, fk_user, mg, tp, ts, status, fk_doc)
values (l_met_klsk, c.vol_id, l_vol, p_user, l_period, 0, p_ts, p_status, p_doc_par_id);
end if;
else
...
select t.fk_k_lsk,
       ...
       p_doc_par_id
from t_objxpar t
where t.fk_k_lsk = l_met_klsk
  and t.mg = l_period
  and t.fk_list = c.vol_id
  and nvl(t.n1, 0) <> 0
group by t.fk_k_lsk, c.vol_id, t.mg, t.tp;
end if;
-- отразить показание
insert into t_objxpar (fk_k_lsk, fk_list, n1, fk_user, mg, tp, ts, status, fk_doc)
values (l_met_klsk, c.num_id, p_n1, p_user, l_period, 0, p_ts, p_status, p_doc_par_id);
...
if p_is_set_prev = 0 then
                            --меняем первую букву на "m" - получаем поле для расхода (изврат)
execute immediate 'update kart k set k.' || 'm' || substr(c.counter, 2, 3) || '=nvl(k.' ||
                                              'm' || substr(c.counter, 2, 3) || ',0) + nvl(' || l_vol ||
                                              + ',0) where k.lsk=:lsk' using c.lsk;
else
                            -- при установке предыдущего показания - занулить расход
execute immediate 'update kart k set k.' || 'm' || substr(c.counter, 2, 3) ||
                                              '=0 where k.lsk=:lsk' using c.lsk;
end if;
exception
                        when others then if sqlcode = -1438 then
rollback;
p_ret := 5;
return;
else
...
return;
                    ...
                    p_ret := 3; -- новые показания меньше или равны существующих в базе
return;
end if;
exit;
end loop;
...
function gen_auto_chrg_all(p_set in number, p_usl in usl.usl%type) return number is
...
if p_set = 1 then --АВТОНАЧИСЛЕНИЕ
        --период, от года назад до прошлого месяца
select to_char(add_months(to_date(p.period || '01', 'YYYYMMDD'), -1 * l_months), 'YYYYMM'),
       to_char(add_months(to_date(p.period || '01', 'YYYYMMDD'), -1), 'YYYYMM'),
       last_day(to_date(p.period || '01', 'YYYYMMDD')), -- последняя дата тек.периода
       p.period
into l_mg1, l_mg2, l_last_dt, l_period
from params p;

for c in (select b.k_lsk_id, b.vol, b.cnt_month_active, b.cnt_month_non_send_vol
                      from (select r.k_lsk_id,
                                   coalesce(m2.vol, 0) as vol,
                                   case
                                       when round(months_between(l_last_dt, r.dt1)) > l_months then l_months
                                       when round(months_between(l_last_dt, r.dt1)) between 1 and l_months
                                           then round(months_between(l_last_dt, r.dt1))
                                       else 0 end as cnt_month_active,                            -- кол-во месяцев, когда счетчик был установлен
                                   (select sum(case when m.max_mg is null then 1 when a.mg >= m.max_mg then 1 else 0 end)
                                    from long_table a
                                    where a.mg between l_mg1 and l_mg2) as cnt_month_non_send_vol -- кол-во месяцев непередачи показаний
                            from kart k
                                     join v_lsk_tp tp
                                          on k.fk_tp = tp.id and tp.cd = 'LSK_TP_MAIN' and k.psch not in (8, 9) and
                                             not exists(select *
                                                        from meter m,
                                                             params p
                                                        where m.fk_klsk_obj = k.k_lsk_id
                                                          and m.fk_usl = p_usl
                                                          and (to_char(m.dt1, 'YYYYMM') = p.period or to_char(m.dt2, 'YYYYMM') = p.period)) -- кроме установленых счетчиков в тек.периоде. ред.27.05.2019
                                     join meter r on k.k_lsk_id = r.fk_klsk_obj and r.fk_usl = p_usl and
                                                     l_last_dt between r.dt1 and r.dt2
                                     left join (select t.fk_k_lsk,
                                                       max(t.mg) as max_mg -- последний месяц, когда были переданы показания
                                                from t_objxpar t,
                                                     u_list s
                                                where t.fk_list = s.id
                                                  and s.cd = 'ins_vol_sch'
                                                  and t.mg >= l_mg1
                                                  and t.tp not in (1, 2) -- без автоначисления
                                                  and t.n1 <> 0
                                                group by t.fk_k_lsk) m on m.fk_k_lsk = r.k_lsk_id
                                     left join (select t.fk_k_lsk,
                                                       sum(t.n1) as vol   -- объем
                                                from t_objxpar t,
                                                     u_list s
                                                where t.fk_list = s.id
                                                  and s.cd = 'ins_vol_sch'
                                                  and t.mg >= l_mg1
                                                  --and t.tp not in (1, 2) -- в том числе автоначисление в прошлых периодах
                                                  and t.n1 <> 0
                                                group by t.fk_k_lsk) m2 on m2.fk_k_lsk = r.k_lsk_id
                                 --where k.k_lsk_id = 104880
                            group by k.lsk, k.k_lsk_id, r.k_lsk_id, r.id, r.dt1, m2.vol, m.max_mg) b
                      where b.cnt_month_non_send_vol > 0
                      order by b.k_lsk_id)
                loop
                    if c.cnt_month_active >= 3 and c.vol > 0 then
                        --не менее 3 месяцев счетчик
                        l_ret2 := ins_vol_meter(p_met_klsk => c.k_lsk_id, p_lsk => null, p_usl => null,
                                                p_vol => round(c.vol / c.cnt_month_active, 3), p_n1 => null, p_tp => 1);
if l_ret2 = 0 then l_ret := 0; end if;
end if;
end loop;
elsif p_set = 0 then
            ...
            for c in (select m.k_lsk_id, t.n1
                ...
                loop
                    ...
                    l_ret2 := ins_vol_meter(p_met_klsk => c.k_lsk_id, p_lsk => null, p_usl => null, p_vol => -1 * c.n1,
                                            p_n1 => null, p_tp => 2);
if l_ret2 = 0 then l_ret := 0; end if;
end loop;
...
commit;
if p_set = 1 then
            logger.log_(null, 'p_meter.gen_auto_chrg_all Окончание-автоначислено по среднему по услуге usl=' || p_usl);
elsif p_set = 0 then
            logger.log_(null, 'p_meter.gen_auto_chrg_all Окончание-Снятие:автоначисления по среднему по услуге usl=' ||
                              p_usl);
end if;
...
for c in (with t as (select distinct d.dat,
                                             case
                                                 when m.fk_usl is not null and m2.fk_usl is not null
                                                     then 1 -- х.в. и г.в.
                                                 when m.fk_usl is not null and m2.fk_usl is null then 2 -- х.в.
                                                 when m.fk_usl is null and m2.fk_usl is not null then 3 -- г.в.
                                                 else 0 end as psch
                             from v_cur_days d
                                      join kart k on k.lsk = p_lsk
                                      join usl u on u.cd = 'х.вода'
                                      join usl u2 on u2.cd = 'г.вода'
                                      left join meter m
                                                on k.k_lsk_id = m.fk_klsk_obj and d.dat between m.dt1 and m.dt2 and
                                                   m.fk_usl = u.usl
                                      left join meter m2
                                                on k.k_lsk_id = m2.fk_klsk_obj and d.dat between m2.dt1 and m2.dt2 and
                                                   m2.fk_usl = u2.usl)
                  select psch, min(dat) as dt1, max(dat) as dt2
                  from (select t.*,
                               row_number() over (order by dat) -
                               row_number() over (partition by psch order by dat) as grp from t)
                  group by grp, psch
                  order by dt1 desc)
            loop
...
for c in (with t as (select distinct d.dat,
                                             case
                                                 when m.fk_usl is not null then 1 -- есть счетчик
                                                 else 0 -- нет счетчика
                                                 end as psch
                             from v_cur_days d
                                      join kart k on k.lsk = p_lsk
...
for c in (select m.id,
                         us.nm,
                         m.fk_usl,
                         k.lsk,
                         m.k_lsk_id,
                         t.fk_meter,
                         months_between(l_dt1, t.dt1) as cnt_months, --кол-во мес. не работы
                         case
                  ...
                  from c_reg_sch t
                           ...
                           join u_list u2
                                on t.fk_state = u2.id and u2.cd = 'Неисправен ПУ' and t.dt1 < l_dt2 -- ограничить тек.периодом
                  ...
                  where months_between(l_dt1, t.dt1) > 0
                    and not exists(select *
                                   from c_reg_sch t2 -- где нет других статусов позднее
                                            ...
                                            join u_list u4
                                                 on t2.fk_state = u4.id and u4.cd in ('Неисправен ПУ', 'Исправен ПУ')
                ...
                if c.cnt_months > 2 then
                    ...
                    for c2 in (select t.n1 * -1 as vol
                        ...
                        loop
                            ...
                            l_ret := ins_vol_meter(p_met_klsk => c.k_lsk_id, p_lsk => null, p_usl => null,
                                                   p_vol => c2.vol, p_n1 => null, p_tp => 4);
if l_ret <> 0 then
...
logger.log_act(c.lsk, '### Неисправный счетчик id=' || c.id || ' по услуге: ' || trim(c.nm) ||
                                          ', >= 3 месяца, установлен норматив', 2);
else
...
logger.log_act(c.lsk, '### Неисправный счетчик id=' || c.id || ' по услуге: ' || trim(c.nm) ||
                                          ', < 3 месяца, снят текущий расход', 2);
end if;
...
--  импорт
for c in (select * from kart k where k.psch not in (8, 9))
            loop
...
procedure imp_states_meter(p_lsk in varchar2,
    ...
    begin
insert into c_reg_sch (dt1, fk_tp, fk_state, text, fk_usl, lsk, fk_meter)
select s.dt1, s.fk_tp, s.fk_state, s.text, null as fk_usl, s.lsk, m.id as fk_meter
from c_reg_sch s
         join u_list u on s.fk_tp = u.id and u.cd = 'Поверка ПУ' and s.fk_usl = p_usl and s.lsk = p_lsk
...
for c in (select s.id,
                         s.lsk,
                         s.fk_status,
                         nvl(s.dt1, l_mfd) as dt1,
                         nvl(s.dt2, l_mld) as dt2,
                         k.k_lsk_id,
                         k.phw,
                         k.pgw,
                         k.pel,
                         case
                             when l_cur_dt2 between nvl(s.dt1, l_mfd) and nvl(s.dt2, l_mld) then 1
                             else 0 end as active,
                         k.sch_el
                  from c_states_sch s
                           join kart k on s.lsk = k.lsk
                  where k.lsk = p_lsk
                  order by nvl(s.dt1, l_mfd), nvl(s.dt1, l_mld))
            loop
                ...
                if c.fk_status = 1 then
                    ...
                    l_met_klsk := ins_meter(p_npp => 1, p_usl => l_usl_hw, p_dt1 => c.dt1, p_dt2 => c.dt2,
                                            p_n1 => case when c.active = 1 then c.phw else null end,
                                            p_fk_klsk_obj => c.k_lsk_id, p_tp => 'ИПУ');
...
l_met_klsk := ins_meter(p_npp => 1, p_usl => l_usl_gw, p_dt1 => c.dt1, p_dt2 => c.dt2,
                                            p_n1 => case when c.active = 1 then c.pgw else null end,
                                            p_fk_klsk_obj => c.k_lsk_id, p_tp => 'ИПУ');
if c.active = 1 then
...
-- счетчики отопления
for c in (select * from kart k where k.psch not in (8, 9) and k.pot <> 0 and k.lsk = p_lsk)
            loop
                l_met_klsk := ins_meter(p_npp => 1, p_usl => l_usl_ot, p_dt1 => l_mfd, p_dt2 => l_mld, p_n1 => c.pot,
                                        p_fk_klsk_obj => l_klsk_obj, p_tp => 'ИПУ');
...
-- счетчики Эл.Эн
for c in (select * from kart k where k.psch not in (8, 9) and k.pel <> 0 and k.sch_el <> 0 and k.lsk = p_lsk)
            loop

                l_met_klsk := ins_meter(p_npp => 1, p_usl => l_usl_el, p_dt1 => l_mfd, p_dt2 => l_mld, p_n1 => c.pel,
                                        p_fk_klsk_obj => c.k_lsk_id, p_tp => 'ИПУ');
...
for c in (select t.sch_el,
            ...
            loop
                ...
                if nvl(c.vol, 0) <> 0 then
                    if p_counter = 'mhw' and (c.psch = 1 or c.psch = 2) then --сч.х.в.
...
elsif p_counter = 'mgw' and (c.psch = 1 or c.psch = 3) then --сч.х.в.
                    --отразить объем на счетчике
insert into t_objxpar (fk_k_lsk, fk_list, n1, mg, fk_user)
values (p_met_klsk, l_vol_id, c.vol, c.mg, l_user);

elsif p_counter = 'mel' and c.sch_el = 1 then --сч.эл.эн.
                    --отразить объем на счетчике
insert into t_objxpar (fk_k_lsk, fk_list, n1, mg, fk_user)
values (p_met_klsk, l_vol_id, c.vol, c.mg, l_user);

elsif p_counter = 'mot' and nvl(c.vol, 0) <> 0 then --сч.отоп.
                    --отразить объем на счетчике
insert into t_objxpar (fk_k_lsk, fk_list, n1, mg, fk_user)
values (p_met_klsk, l_vol_id, c.vol, c.mg, l_user);
...
met_klsk :=
                ins_meter(p_npp => 1, p_usl => '011', p_dt1 => dt1, p_dt2 => dt2, p_n1 => 0, p_fk_klsk_obj => l_klsk,
                          p_tp => 'ИПУ');

        --   insert into t_objxpar (fk_k_lsk, fk_list, n1)
        --     values(met_klsk, 2694, 5.56);
insert into t_objxpar (fk_k_lsk, fk_list, n1) values (met_klsk, 2693, 555);
update meter t set t.n1 = 555 where t.k_lsk_id = met_klsk;
update kart k set k.phw = 555, k.mhw = null where k.k_lsk_id = l_klsk;

met_klsk :=
                ins_meter(p_npp => 1, p_usl => '015', p_dt1 => dt1, p_dt2 => dt2, p_n1 => 0, p_fk_klsk_obj => l_klsk,
                          p_tp => 'ИПУ');

        --   insert into t_objxpar (fk_k_lsk, fk_list, n1)
        --     values(met_klsk, 2694, 12);
insert into t_objxpar (fk_k_lsk, fk_list, n1) values (met_klsk, 2693, 777);
