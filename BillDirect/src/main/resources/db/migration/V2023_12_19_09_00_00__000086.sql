create or replace package body   P_GIS is
...
-- добавить недостающие ПД по всем УК (обычно для ТСЖ), за текущий период
function insert_pd_all_uk(p_period in varchar2 default null) -- период загрузки, может быть не задан)
   return number is
  l_cnt number;
begin
  -- по каждому УК
l_cnt:=0;
for c in (select t.* from exs.eolink t join bs.addr_tp tp on t.fk_objtp=tp.id
                    and tp.cd='Организация' and t.reu is not null) loop
    l_cnt:=l_cnt+insert_pd_by_uk(c.id, p_period);
end loop;
return l_cnt;
end;

-- добавить недостающие ПД по УК
function insert_pd_by_uk(p_eol_uk in number, -- Id УК в Eolink
                         p_period in varchar2 default null -- период загрузки, может быть не задан
                        ) return number is
  l_cnt number;
begin
  -- по каждому дому
...
for c in (select t.id from exs.eolink t where t.parent_id=p_eol_uk) loop
    l_cnt:=l_cnt+insert_pd_by_house(c.id, null, p_eol_uk, p_period);
end loop;
return l_cnt;
end;
...
select last_day(scott.getdt(1, substr(p.s1, 5, 2), substr(p.s1, 1, 4)))
into l_dt
from exs.eolink t
         join exs.eolxpar p on t.id = p.fk_eolink
         join oralv.u_hfpar u on p.fk_par = u.id
where t.fk_objtp = 1
  and t.parent_id is null
  and u.cd = 'ГИС ЖКХ.PERIOD_IMP_PD';
else
      l_dt := last_day(scott.getdt(1, substr(p_period, 5, 2), substr(p_period, 1, 4)));
end if;
...
rollback;
Raise_application_error(-20000, 'Отмена! Уже выполняется задание на загрузку ПД');
end if;
end if;
return l_cnt;
end;
...
select k.reu, count(*) as cnt, max(k.lsk) as lsk, k.reu, k.kul, s.name as street,
       ltrim(k.nd,'0') as nd from scott.kart k
                                      join scott.spul s on k.kul=s.id
                                      join scott.v_lsk_tp tp on k.fk_tp=tp.id and tp.cd in ('LSK_TP_MAIN', 'LSK_TP_RSO')
where k.psch not in (8,9)
  and exists (
...
-- изменить Id УК(РСО) в заданиях определенного типа (например выгрузки приборов учета)
function change_reu_in_task_by_house(p_eol_house in number, -- Id дома в Eolink
                             p_reu in exs.eolink.reu%type, -- код REU
                             p_act_cd in varchar2 -- cd задачи, например 'GIS_EXP_METERS'
                             ) return number is
 l_cnt number;
l_proc_uk number;
l_act number;
begin
select o.id into l_proc_uk from exs.eolink o
                                    join bs.addr_tp tp on o.fk_objtp=tp.id and tp.cd='Организация'
    and o.reu=p_reu;

select t.id into l_act from bs.list t where t.cd=p_act_cd;

update exs.task t set t.fk_proc_uk=l_proc_uk
where t.fk_eolink=p_eol_house and t.fk_act=l_act;
l_cnt:=sql%rowcount;
