            ...
            loop
                for c2 in (select t.n1
                           from t_objxpar t
                                    ...
                                    join v_params p on t.mg = p.period3
...
if /*p_control in (0, 2) or */p_n1 >= l_last_n1 then -- обработка новых показаний
                -- получить объем
                    l_vol := nvl(p_n1, 0) - nvl(l_n1, 0);
--if p_control in (1, 2) then
...
--if l_vol < 0 then
-- Ошибка! переданы показания, вне диапазона, - сняли, меньше чем было показание на начало периода
--                          p_ret := 7;
--return;
--end if;
--end if;
...
for c in (select b.k_lsk_id, b.vol, b.cnt_month_active, b.cnt_month_non_send_vol
                      from (select r.k_lsk_id,
                            ...
                            group by k.lsk, k.k_lsk_id, r.k_lsk_id, r.id, r.dt1, m.vol, m.max_mg) b
