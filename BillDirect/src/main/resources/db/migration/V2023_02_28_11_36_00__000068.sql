...
for c in (select k.lsk, m.id, m.fk_usl, trim(u3.counter) as counter, u.id as vol_id, u2.id as num_id
            ...
            loop
                for c2 in (select t.n1
                           ...
                           where t.fk_k_lsk = l_met_klsk -- последние показания счетчика, отправленные в прошлом периоде
                           order by t.id desc)
                    loop
                        l_last_n1 := c2.n1;
l_n1 := l_last_n1;
...
for c2 in (select t.n1
                           from t_objxpar t
                                    join u_list u on t.fk_list = u.id and u.cd = 'ins_sch'
                                    join params p on t.mg = p.period
                           where t.fk_k_lsk = l_met_klsk -- последние показания счетчика, отправленные в текущем периоде
                           order by t.id desc)
                    loop
                        l_n1 := c2.n1;
exit;
