create or replace package scripts is
...
procedure dist_saldo_polis(p_dt in date) ;
procedure dist_saldo_PEN_polis(p_dt in date) ;
procedure swap_chrg_pay_by_one_org;
procedure swap_sal_to_another_org;
procedure swap_pen_to_pay;
end scripts;
/
  ...
  while l_diff<>0 and l_flag_dist=true loop
      ...
      for c2 in (select usl, org, sum(summa) as summa from
        (select t.usl, t.org, abs(t.summa) as summa
            from saldo_usl t where t.mg=c.mg and
