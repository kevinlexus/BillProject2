...
public class UlistMng {
    ...
    private void updNsiList(Integer taskId, List<UlistTp> lst, NsiItemInfoType nsiItem, String grp) throws CantUpdNSI {
        ...
        if (!el.isPresent()) {
            ...
            UlistTp lstTp = new UlistTp();
            lstTp.setCd(prefix);
            lstTp.setFkExt(nsiItem.getRegistryNumber().intValue());
            lstTp.setName(nsiItem.getName());
            lstTp.setDt1(Utl.getDateFromXmlGregCal(nsiItem.getModified()));
            lstTp.setGrp(grp);

            //ulistTpDAO.save(lstTp);
            em.persist(lstTp);
            log.info("Создан заголовочный элемент U_LIST_TP Id={}, prefix={}", lstTp.getId(), prefix);
    ...
    public Integer mergeElement(UlistTp ulistTp, String grp, Integer id, NsiElementType t, Integer idx, String org) {
        ...
        for (NsiElementFieldType d : lst2) {
            ...
            if (d.getClass().equals(NsiElementStringFieldType.class)) {
                ulist = createField(ulistTp, grp, id, t, idx, org, main, (NsiElementStringFieldType) d);
            } else if (d.getClass().equals(NsiElementNsiRefFieldType.class)) {
                ulist = createRefField(ulistTp, grp, id, t, idx, org, main, (NsiElementNsiRefFieldType) d, ulist);
            }
    ...
    private Ulist createRefField(UlistTp ulistTp, String grp, Integer id, NsiElementType t, Integer idx, String org, Ulist main, NsiElementNsiRefFieldType d, Ulist ulist) {
        String fldCd;
        NsiElementNsiRefFieldType fld = d;
    ...
    private Ulist createField(UlistTp ulistTp, String grp, Integer id, NsiElementType t, Integer idx, String org, Ulist main, NsiElementStringFieldType d) {
        Ulist ulist;
        String fldCd;
        NsiElementStringFieldType fld = d;
        // создать запись в Ulist
        String name;
        if (fld.getName() == null || fld.getName().length() == 0) {
            name = "------";
