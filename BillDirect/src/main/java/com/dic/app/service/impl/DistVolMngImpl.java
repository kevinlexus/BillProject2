...
import com.dic.app.service.DistVolMng;
...
import static com.dic.app.service.impl.enums.ProcessTypes.CHARGE_FOR_DIST_2;
        ...
        try {
            ...
            if (tp != -1) {
                ...
                if (kub.compareTo(BigDecimal.ZERO) != 0) {
                    ...
                    reqConf2.setTp(CHARGE_FOR_DIST_2); // начисление для распределения по вводу
                    reqConf2.prepareId();
                                    ...
                                    if (diff.compareTo(BigDecimal.ZERO) != 0 && amnt.areaAmnt.compareTo(BigDecimal.ZERO) != 0) {
                                        ...
                                        if (diff.compareTo(BigDecimal.ZERO) > 0) {
                                            ...
                                            while (iter.hasNext()) {
                                                ...
                                                //                                                Set<Nabor> setNabor = uslVolKartGrp.getKart().getKoKw().getKart()
                                                //                                                        .stream().flatMap(t -> t.getNabor().stream()).collect(Collectors.toSet());
